...
for connector in data:
    ...
    # Check if the connector is in transition
    if available and (status == 'pending_start' or status == 'pending_stop'):
        # Add the connector and hostname to connectors_in_transition
        connectors_in_transition[connector_guid] = hostname

# Print the number of connectors found in the first 500 results
print('Connectors found: {}'.format(len(connectors_in_transition)))
...
while 'next' in response_json['metadata']['links']:
    ...
    connectors_in_transition.update(additional_connectors)

# Print the total number of connectors found
print('Total connectors found that are in transition: {}'.format(
    len(connectors_in_transition)))

# Write the GUIDs to a file named 'connectors_in_transition.txt'
if connectors_in_transition:
    print('\nWriting connector GUIDs to connectors_in_transition.txt')
    with open('connectors_in_transition.txt', 'w') as file:
        # Iterate over the stored GUIDs
        for guid in connectors_in_transition:
